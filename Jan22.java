...
class Solution
{
    ...
    public static void dfs(Node node,int target_sum,ArrayList<Integer> currentPath,ArrayList<ArrayList<Integer>> result){
        if(node == null){
            return;
        }
        currentPath.add(node.data);
        if(target_sum == node.data){
            result.add(new ArrayList<>(currentPath));
        }
        dfs(node.left,target_sum - node.data,new ArrayList<>(currentPath),result);
        dfs(node.right,target_sum - node.data, new ArrayList<>(currentPath),result);
    }
}